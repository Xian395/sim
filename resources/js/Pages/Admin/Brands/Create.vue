      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
           <div class="flex items-center justify-between border-b border-stroke py-4 px-6.5 dark:border-strokedark p-6">
              <h2 class="text-2xl font-black text-blue-700">Add New Brand</h2>
              <ButtonNew
                types="back"
                size="md"
                tooltips="return to brand list"
...
const submit = async () => {
  try {
    const response = await axios.post(route('admin.brands.store'), form.data());

    notify('Brand saved successfully.', 'success');
    form.reset();
